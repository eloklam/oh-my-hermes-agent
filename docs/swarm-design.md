...
3. **Deliberate oracle/model review follow-ups** — Oracle or other model review follow-ups are allowed when explicit, high-value, or part of an approved run. After a blocker resolves, checkpoint before starting silent repeated expensive review cascades, unless the user already approved full auto-run and accepted the cost.
4. **Use deep review where it fits** — Use oracle/deep-review for explicit requests, final or high-risk gates, architecture/debug review, and high-stakes decisions. For normal verification, prefer deterministic local checks or lower-cost worker profiles; avoid chaining multiple expensive reviews silently.
